  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={() => onChange(Math.max(0, value - 1))}
        className="w-6 h-6 rounded-[6px] bg-[#F2F2F7] font-bold text-sm flex items-center justify-center">−</button>
...
export default function CheckinPage() {
  ...
  useEffect(() => {
    supabase.from('pets').select('id').limit(1).single().then(({ data }) => {
      if (data) { setPetId(data.id); loadCheckin(data.id, selectedDate) }
      else setLoading(false)
    })
  }, [])

  async function loadCheckin(pid: string, date: string) {
    setLoading(true)
    setSaved(false)
    resetForm()
  ...
  async function save() {
    ...
    setSaving(false)
    setSaved(true)
    if (petId) loadCheckin(petId, selectedDate)
  }
    ...
    <div className="min-h-screen bg-[#F2F2F7] flex flex-col pb-16">
      ...
      <div className="px-3 py-3 flex flex-col gap-3">
        ...
        {loading ? (
          ...
          // Чек-ин уже заполнен — показываем данные
          <>
            {saved && (
          ...
          // Сегодня, чек-ин не заполнен — форма
          <>
            ...
            <div className="bg-white rounded-[13px] border border-[#E5E5EA] p-[10px_11px]">
              ...
              <div className="text-[8px] font-bold text-[#8E8E93] uppercase tracking-wide mb-1.5">Характер</div>
              <TapGrid options={STOOL_TYPE_OPTIONS} value={stoolType} onChange={setStoolType} cols={3} />
              <div className="text-[8px] font-bold text-[#8E8E93] uppercase tracking-wide mb-1.5 mt-2">Запах</div>
            ...
            <div className="bg-white rounded-[13px] border border-[#E5E5EA] p-[10px_11px]">
              ...
              <div className="text-[8px] font-bold text-[#8E8E93] uppercase tracking-wide mb-1.5">Объём</div>
          ...
          // Прошлый день, данных нет
          <div className="text-center py-12 text-[#8E8E93]">
            <p className="text-3xl mb-2">📋</p>
            <p className="text-sm font-medium">Чек-ин за этот день не заполнен</p>
            <p className="text-[10px] mt-1">Данные доступны только за сегодня</p>
          </div>
        )}
      </div>
