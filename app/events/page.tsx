...
export default function EventsPage() {
  ...
  useEffect(() => {
    supabase.from('pets').select('id').limit(1).single().then(({ data }) => {
      if (data) { setPetId(data.id); loadEvents(data.id, selectedDate) }
  ...
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === 'visible' && petId) loadEvents(petId, selectedDate)
    }
    document.addEventListener('visibilitychange', handleVisibility)
    return () => document.removeEventListener('visibilitychange', handleVisibility)
  }, [petId, selectedDate])

  async function loadEvents(pid: string, date: string) {
    setLoading(true)
  ...
  const selectedLabel = format(new Date(selectedDate), 'EEEE, d MMMM', { locale: ru })
